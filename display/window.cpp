...
void Window::initialize(const WindowDesc &description) {
    ...
    if (description.fullscreen) {
        _handle = glfwCreateWindow(static_cast<int>(description.size.width),
...
void Window::setPosition(const Offset2D &position) {
...
void Window::setSize(const Extent2D &size) {
    glfwSetWindowSize(_handle, static_cast<int>(size.width),
